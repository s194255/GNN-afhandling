...
class VisNetSelvvejledtPL(L.LightningModule):
    ...
    def __init__(self, visnetdownstream: VisNetDownstream, debug: bool):
        super().__init__()
        self.hovedmodel = visnetdownstream
    ...
    def forward(
        ...
        y, dy = self.hovedmodel(z, pos, batch)
        return y, dy

    def training_step(self, data: Data, batch_idx: int) -> torch.Tensor:
        pred_y, pred_dy = self(data.z, data.pos, data.batch)
        return self.criterion(pred_y, data.y)
