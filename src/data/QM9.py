...
class QM9ByggerForældet:
    ...
    def __init__(self, delmængdestørrelse: float = 0.1, fordeling = None, batch_size=32, num_workers=0):
        ...
        self.data_splits_path = QM9ByggerForældet.data_splits_path
    ...
    @staticmethod
    def reset():
        if os.path.exists(QM9ByggerForældet.data_splits_path):
            os.remove(QM9ByggerForældet.data_splits_path)
...
class QM9Bygger(L.LightningDataModule):
    ...
    def setup(self, stage: str) -> None:
        ...
        for task in self.get_setup_tasks(stage):
            self.datasets[task] = torch.utils.data.Subset(mother_dataset, self.data_splits[self.trainer.model.debug][task])
